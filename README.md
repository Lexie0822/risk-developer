...
项目内置完整的单元测试、集成测试与性能基准，覆盖所有核心需求与扩展点。

### 1. 运行全部单元/集成测试

```bash
# 依赖安装完成后一键执行
pytest -q tests             # 推荐，如未安装 pytest 可使用 python -m unittest
# 或
...
1. 引擎能够正确执行风控逻辑并生成对应 Action。
2. 扩展维度、动态阈值、每日重置等扩展点均得到验证。

### 2. 性能基准

基准脚本位于 `bench.py`（同步） 与 `bench_async.py`（异步）。

```bash
python bench.py          # 同步基准，默认 20 万事件
python bench_async.py    # 异步基准，默认 100 万事件
```

输出示例：

```text
Processed 1000000 events in 0.85s => 1 176 470 evt/s
```

若 **吞吐 ≥ 1,000,000 evt/s** 且 **P99 延迟 ≤ 1,000 µs**，说明已达到性能目标。
...
该脚本演示了引擎初始化、订单/成交输入、Action 结果以及动态规则更新。

> 提示：可在脚本中自行修改阈值、维度映射，验证扩展点兼容性。
